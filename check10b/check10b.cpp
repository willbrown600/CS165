...
void promptInventory(vector<Food*> &items)
{
   ...
   do
   {
      ...
      if (name != "quit")
      {
         ...
         Food * product = new Food(name, month, year);
...
void displayInventory(vector<Food*> &items)
{
   ...
   for (vector<Food*> :: iterator it = items.begin();
         ...
         {
            //Food * items = it;
            (*it)->display();       //dereference each item of it through display function
                        ...
                        int currentYear)
{
   ...
   for ( vector<Food*> :: iterator it = items.begin();
         it != items.end();)    //I took the i++ out of the for loop as it accidentally tried
   {                            //to advance onto the '/0' for the destructor causing me problems. 
      ...
      Food * pItem = *it;
      if((pItem->getMonth() < currentMonth) && 
          pItem->getYear() <= currentYear )
      {    
         delete pItem;           //engage destructor.
         pItem = NULL;           //ensure memory freed up.
         it = items.erase(it);   //erase it from the vector.
         //From the code below the erase automatically advances the iterator without use of a i++. 
      }
      else
      {
         it++;   
